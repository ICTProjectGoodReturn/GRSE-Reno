...
public class LenderPersistenceImpl extends BasePersistenceImpl<Lender>
	...
	private static final String _SQL_GETPERSONS = "SELECT {WEAL_Person.*} FROM WEAL_Person INNER JOIN WEAL_Lender ON (WEAL_Lender.lender_Id = WEAL_Person.lender_Id) WHERE (WEAL_Lender.lender_Id = ?)";
	private static final String _SQL_GETPERSONSSIZE = "SELECT COUNT(*) AS COUNT_VALUE FROM WEAL_Person WHERE lender_Id = ?";
	private static final String _SQL_CONTAINSPERSON = "SELECT COUNT(*) AS COUNT_VALUE FROM WEAL_Person WHERE lender_Id = ? AND abacus_Person_Id = ?";
